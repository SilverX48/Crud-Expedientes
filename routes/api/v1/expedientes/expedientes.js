...
router.post('/new', async (req, res) => {
  ...
  try {
    rslt = await expedienteModel.new(identidad, fecha, descripcion, observacion, registros, ultimoActualizacion);
    res.status(200).json(
      {
        status: 'ok',
        result: rslt
...
router.put('/update/:id', async (req, res) => {
  try{
    const {identidad, fecha, descripcion, observacion, registros, ultimoActualizacion } = req.body;
    const { id } = req.params;
    const result = await expedienteModel.updateOne(id, identidad, fecha, descripcion, observacion, registros, ultimoActualizacion);
    res.status(200).json({
      status:'ok',
      result
    });
